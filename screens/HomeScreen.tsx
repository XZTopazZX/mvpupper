...
export default function HomeScreen() {
  ...
  const logActivity = async (activityType: string) => {
    ...
    try {
      ...
      Alert.alert('Logged!', `${activityLabel} recorded for ${household?.dogs.find(d => d.id === selectedDogId)?.name}`);
    } catch (error) {
    ...
    <ScrollView style={styles.container}>
      ...
      {!isPremium && household && household.dogs.length >= 2 && (
        <View style={styles.premiumBanner}>
          ...
          <Text style={styles.premiumText}>Unlock unlimited dogs, medical tracking, and more!</Text>
